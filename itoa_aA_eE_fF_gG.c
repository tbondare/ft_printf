...
int cnt_till_aA_eE_fF_gG(int base, long double *mem_val)
{
  ...
  *mem_val =  *mem_val < 0 ? -  *mem_val :  *mem_val;
  if (*mem_val >= 1)
  {
	  while (*mem_val >= 1)
	  {
		  *mem_val = *mem_val / base; // - считаю количество знаков до зап€тых.
		  cnt++;
	  }
  }
  else
  {
	  while (*mem_val < 1)
	  {
		  *mem_val = *mem_val * base; // узнаю количество нулей до зап€той дл€ экспоненты, потому что при экспоненте удал€ютс€ нули
		  cnt++;
	  }
